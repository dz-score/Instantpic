...
# The mock_gphoto2 fixture in conftest.py mocks the gphoto2 module; gp is
# imported in exactly one place (backend.camera.device), so tests patch there.
...
def test_resume_refused_while_capture_in_flight(mock_gphoto2):
    """The gate is the single owner of the preview-vs-capture rule: a
    preview-stream reconnect (resume) must not re-arm live view while a
    capture is pending or running."""
    with patch('backend.camera.device.gp', mock_gphoto2):
        from backend.camera.service import CameraService
        camera = CameraService(MagicMock())

        camera._gate.begin_capture()
        assert camera._gate.preview_armed() is False

        camera.resume_preview()
        assert camera._gate.preview_armed() is False, \
            "resume re-armed live view mid-capture — the gate must refuse"

        camera._gate.end_capture()
        camera.resume_preview()
        assert camera._gate.preview_armed() is True
...
def test_enqueue_capture(mock_gphoto2):
    with patch('backend.camera.device.gp', mock_gphoto2):
        ...
        with patch.object(camera._runner, '_emit_job_state') as mock_emit:
            job_id = camera.enqueue_capture()

            assert job_id is not None
            assert len(job_id) == 8

            # Should have been placed in the runner's queue
            assert not camera._runner._queue.empty()
            cmd = camera._runner._queue.get()
            assert cmd["job_id"] == job_id

            # The capture must be authoritative before the job is serviced:
            # preview paused, gate closed against resume.
            assert camera._gate.capture_in_progress is True
            assert camera._gate.preview_armed() is False
...
def test_execute_capture_job_retries_once_then_succeeds(mock_gphoto2):
    ...
    with patch('backend.camera.device.gp', mock_gphoto2):
        ...
        with patch.object(camera._preview, 'start_worker'):
            camera.init()
        camera.connected = True  # capture() failure sets this False; force back for the retry

        job_id = "test_job_retry"

        with patch('backend.camera.capture.time.sleep') as mock_sleep, \
             patch.object(camera._device, 'init') as mock_init, \
             patch.object(camera._runner, '_emit_job_state') as mock_emit:
            mock_init.side_effect = lambda: setattr(camera._device, 'connected', True)

            camera._runner.execute(job_id)

            # One pre-capture preview-release settle + one retry delay.
            mock_sleep.assert_any_call(camera._runner.PREVIEW_RELEASE_SETTLE_S)
            mock_sleep.assert_any_call(camera._runner.CAPTURE_RETRY_DELAY_S)
