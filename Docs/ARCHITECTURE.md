...
- **Wedged-session heal**: If the init-time warmup preview fails (the signature of a live-view state the M50 is left in after the previous run took a photo — config reads work but every preview stalls ~3s then errors `[-1]`; it survives a clean exit, see CAMERA_NOTES §2), the worker heals it: the wedge clears only after ~2 stalls of polling followed by an `exit()+init()`. The warmup grab is stall #1, so the worker re-inits after **1** more stall (`error_limit=1` when `_warmup_failed`) → warmup succeeds. Heal completes ~9s after boot, while the booth is still on the attract screen, so the guest's first capture works.
