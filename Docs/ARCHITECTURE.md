...
- **Wedged-session heal**: If the init-time warmup preview fails (the signature of a live-view state the M50 is left in after the previous run took a photo — config reads work but every preview stalls ~3s then errors `[-1]`; it survives a clean exit, see CAMERA_NOTES §2), the worker gives up after 2 consecutive errors instead of 6 and re-inits. Re-init does **not** cure the wedge — it self-clears in ~13–20s; the fail-fast just keeps the session alive until the camera recovers, while the booth is still on the attract screen.
