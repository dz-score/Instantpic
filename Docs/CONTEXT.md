...
| [`camera_provider.py`](backend/camera_provider.py) | `create_camera(settings)` — a factory that returns the backend named by the config (`gphoto2` or `mock`), or `None` when gphoto2 is not installed. Importing it must not pick or open a camera; the lifespan calls it once. |
