...
| [`PRINTER_NOTES.md`](PRINTER_NOTES.md) | DNP DS-RX1HS field knowledge, what is still assumed rather than measured, and the hardware-run checklist. |
...
| [`state_machine.py`](backend/state_machine.py) | Event-driven FSM representing the booth's logical state (`BoothState`). Validates transitions, updates state, and broadcasts changes via SSE. Takes its SSE service, job queue and camera as constructor arguments from the lifespan — it imports none of them. |
...
| [`camera/`](backend/camera/) | Camera package, split by responsibility: `service.py` (facade — the surface the app talks to), `device.py` (gphoto2 handle, camera lock, every locked USB primitive; sole importer of `gphoto2`), `preview.py` (worker thread, frame buffer, MJPEG generator, idle watchdog, metrics monitor), `capture.py` (capture jobs, retry-once policy, FSM callbacks), `gate.py` (the one owner of the "no preview while a capture is pending" rule). |
| [`camera/mock.py`](backend/camera/mock.py) | Drop-in replacement for the camera facade that generates synthetic preview frames; used on Windows/dev when `camera_backend: "mock"` is set in config. |
| [`photo_processor.py`](backend/photo_processor.py) | Composites captured images into the 1800×1200 px print canvas using Pillow: arranges single/collage layouts, blends RGBA overlay PNGs, renders Playfair Display branding text, and saves as JPEG tagged at 300 DPI. Also draws the printer alignment card (`generate_alignment_card`). |
| [`print_service.py`](backend/print_service.py) | Abstracts printer hardware behind a `PrinterDriver` ABC with `CupsPrinterDriver` (Linux, `lp`/`lpstat`/`ipptool`) and `MockPrinterDriver` (dev, shaped like a DS-RX1HS). Submission and completion are separate calls — `print_file()` then `await_job()` — so "printed" means paper, not acceptance. `PrintService` adds the submit-only retry, the low-media threshold, status caching and structured logging; it is constructed by the lifespan with the `SettingsService` so a printer swap in the admin panel takes effect without a restart. |
| [`sse_service.py`](backend/sse_service.py) | Fan-out Server-Sent Events service. Built by the lifespan and injected into the FSM, the camera and the config route, which call `dispatch_event()` to push state updates, camera status, and capture results to all connected browser clients simultaneously. |
...
| [`settings.py`](backend/settings.py) | Pydantic `AppSettings` schema, atomic `read_settings()` / `write_settings()` for `config.json` at the project root, and `SettingsService`, which owns the live settings for the process. The composition root (`main.py` lifespan) builds it; nothing reaches for it through a module global. |
| [`deps.py`](backend/deps.py) | FastAPI dependencies that hand routes the services the lifespan built (settings, printer, camera), read off `app.state`. Routes ask for what they need instead of importing a service singleton. |
| [`camera/factory.py`](backend/camera/factory.py) | `create_camera(settings)` — a factory that returns the backend named by the config (`gphoto2` or `mock`), or `None` when gphoto2 is not installed. Importing it must not pick or open a camera; the lifespan calls it once. |
...
| [`diagnostics.py`](backend/diagnostics.py) | Aggregates system health (printer status, disk usage, photo count) for the admin panel, and executes emergency actions (`restart_booth`, `restart_printer`, `clear_queue`) — printer ones through `PrintService`, the rest via systemd. |
| [`tools/check_duplication.py`](backend/tools/check_duplication.py) | Reports prose written down in more than one file — the mechanical half of BACKEND_RULES Rule 25. `--since <ref>` subtracts what already overlapped, so a branch only sees what it introduced. |
| [`tools/printer_markers_probe.py`](backend/tools/printer_markers_probe.py) | Bench script: dumps a queue's raw CUPS marker attributes beside what `CupsPrinterDriver._read_media()` makes of them, so the media parser can be corrected against a real DS-RX1HS. Delete once that question is answered (Rule 24). |
...
| [`test_settings.py`](backend/tests/test_settings.py) | Tests for `read_settings` / `write_settings` round-trips, the atomic-write and corrupt-config guarantees, and `SettingsService` (rebind-not-mutate, load-before-get, per-instance isolation). |
