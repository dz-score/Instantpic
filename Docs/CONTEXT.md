...
| [`print_service.py`](backend/print_service.py) | Abstracts printer hardware behind a `PrinterDriver` ABC with `CupsPrinterDriver` (Linux, `lp` CLI) and `MockPrinterDriver` (dev). `PrintService` adds retry logic, status caching, and structured logging; it is constructed by the lifespan with the `SettingsService` so a printer swap in the admin panel takes effect without a restart. |
