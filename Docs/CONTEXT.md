...
| [`counters.py`](backend/counters.py) | Named integer tallies the booth keeps across restarts (currently just `prints_used`). Separate from `AppSettings` because the lifecycles differ: settings are operator decisions worth committing, these are numbers the booth writes to itself mid-event. Persisted to `backend/state/counters.json`, which is gitignored. |
